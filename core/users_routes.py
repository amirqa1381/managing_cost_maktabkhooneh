...
async def sign_up_user(request: UserRegisterSchema, db: Session = Depends(get_db)):
    ...
    if user is not None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This username is already exists")
...
async def loin(response: Response, request:OAuth2PasswordRequestForm , db: Session = Depends(get_db)):
    """
...
def refresh_token(response: Response,refresh_token: str = Cookie(None), db:Session = Depends(get_db)):
    ...
    stored = db.query(RefreshToken).filter_by(token=refresh_token, revoked=False).first()
    if not stored:
